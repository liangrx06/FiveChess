...
if __name__ == "__main__":
    ...
    path = r'D:\Desktop\1.png'  # 需要保存到E盘的目录文件名wq

    # 整个屏幕截图，也可以指定x、y坐标进行截图
    img0 = ImageGrab.grab(bbox=(0, 0, 600, 600))
    # img0 = ImageGrab.grab()  # 也可以不传参数，默认截取整个屏幕
    img0.save(path)  # 保存

    img = ac.imread(path)  # 需要用aircv转换，方便cv2.imshow函数打开

    # img = cv2.imread("./screen/9.jpg")
    img = np.array(img0.getdata(), np.uint8).reshape(img0.size[1], img0.size[0], 3)
    start_time = time.time()
    '''********************************************
    1、定位棋盘位置
    ********************************************'''
    img_after = dingweiqizi_weizhi(img0)
    time1 = time.time()
    print(time1 - start_time)
    # cv2.imshow("src", img)
    # cv2.waitKey()
    # cv2.imshow("src", img_after)
    # cv2.waitKey()
